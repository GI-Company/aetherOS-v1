...
func wsHandler(w http.ResponseWriter, r *http.Request) {
	...
	for {
		...
		if mt == websocket.TextMessage {
			...
			go handleClientJSON(sid, env, conn)
		} else {
			// binary frames -> forward to wasm as audio chunk
			go handleClientBinary(sid, msg)
		}
	}
}

func handleClientJSON(sid string, env map[string]interface{}, conn *websocket.Conn) {
...
func handleClientBinary(sid string, msg []byte) {
